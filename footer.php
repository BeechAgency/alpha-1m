<?php
/**
 * The template for displaying the footer
 *
 * Contains the closing of the #content div and all content after.
 *
 * @link https://developer.wordpress.org/themes/basics/template-files/#template-partials
 *
 * @package Beech_ACF_Boilerplate
 */

	$fields = array(
        'footer_links',
        'social_links',
        'footer_terms'
    );

	$front_page_id = get_option('page_on_front');

    foreach($fields as $field) {
        $$field = get_field($field, $front_page_id);
    }

?>

	<footer id="colophon" class="site-footer" data-scroll data-scroll-call="sectionScroll"  data-scroll-repeat data-section-order="7">
		<div class="site-footer__row container has-gutter flex-row space-between nav-row">
			<div class="footer-logo flex-row">
				<div class="alpha-logo">
					<svg xmlns="http://www.w3.org/2000/svg" width="56" height="50" viewBox="0 0 56 50" fill="none">
					<path d="M8.45058 33.0479C3.78319 33.0479 0.00146635 36.8425 0 41.5241C0 46.2049 3.78319 50.0003 8.45058 50.0003C13.118 50.0003 16.9034 46.2056 16.9034 41.5241C16.9034 36.8425 13.1187 33.0479 8.45058 33.0479Z" fill="white"/>
					<path d="M48.9905 6.85344C40.1887 -1.97649 27.8772 -2.29565 19.5835 6.02097L18.4991 7.10862L29.0891 17.73C33.1707 13.7625 35.7867 13.3154 38.5295 16.0665C40.699 18.2418 40.5714 20.673 38.2751 22.977C36.4246 24.8325 33.681 25.5362 28.004 25.4722C19.5835 25.2156 15.246 26.368 12.0567 29.6956L21.9436 39.6125C24.0493 37.885 26.3449 37.5012 32.788 37.6931C38.4657 37.885 40.1887 37.8203 42.9315 37.1173C45.8011 36.4135 48.4809 34.8773 50.5213 32.8306C57.3472 25.9841 56.7086 14.5957 48.9905 6.85344Z" fill="white"/>
					</svg>
				</div>
				<div class="star-logo">
					<svg xmlns="http://www.w3.org/2000/svg" width="51" height="50" viewBox="0 0 51 50" fill="none" class="spin">
						<path d="M17.231 1.18777L13.7777 2.61816L32.9119 48.8121L36.3651 47.3817L17.231 1.18777Z" fill="url(#paint0_linear_2421_900)"/>
						<path d="M8.7154 6.00053L6.07237 8.64355L41.4277 43.9989L44.0707 41.3559L8.7154 6.00053Z" fill="url(#paint1_linear_2421_900)"/>
						<path d="M2.68963 13.7069L1.25923 17.1602L47.4532 36.2943L48.8836 32.8411L2.68963 13.7069Z" fill="url(#paint2_linear_2421_900)"/>
						<path d="M0.0714264 23.1313L0.0714264 26.8691L50.0714 26.8691V23.1313L0.0714264 23.1313Z" fill="url(#paint3_linear_2421_900)"/>
						<path d="M1.25906 32.8407L2.68945 36.2939L48.8834 17.1598L47.453 13.7065L1.25906 32.8407Z" fill="url(#paint4_linear_2421_900)"/>
						<path d="M6.07196 41.356L8.71499 43.999L44.0703 8.64368L41.4273 6.00066L6.07196 41.356Z" fill="url(#paint5_linear_2421_900)"/>
						<path d="M13.7779 47.3821L17.2312 48.8125L36.3653 2.61852L32.9121 1.18813L13.7779 47.3821Z" fill="url(#paint6_linear_2421_900)"/>
						<path d="M26.9403 0H23.2025V50H26.9403V0Z" fill="#E42312"/>
						<defs>
							<linearGradient id="paint0_linear_2421_900" x1="15.8873" y1="3.38707" x2="34.2462" y2="46.5918" gradientUnits="userSpaceOnUse">
							<stop stop-color="#41B36E"/>
							<stop offset="0.05" stop-color="#41B583"/>
							<stop offset="0.13" stop-color="#42BAA5"/>
							<stop offset="0.22" stop-color="#43BDBF"/>
							<stop offset="0.31" stop-color="#44C0D2"/>
							<stop offset="0.41" stop-color="#44C1DE"/>
							<stop offset="0.51" stop-color="#45C2E2"/>
							<stop offset="1" stop-color="#FBA5E4"/>
							</linearGradient>
							<linearGradient id="paint1_linear_2421_900" x1="8.13535" y1="8.33993" x2="41.886" y2="41.5407" gradientUnits="userSpaceOnUse">
							<stop stop-color="#E4BC81"/>
							<stop offset="0.08" stop-color="#E6A56E"/>
							<stop offset="0.19" stop-color="#E98B58"/>
							<stop offset="0.31" stop-color="#EB7848"/>
							<stop offset="0.43" stop-color="#EC6C3F"/>
							<stop offset="0.56" stop-color="#ED693C"/>
							<stop offset="1" stop-color="#F8A91F"/>
							</linearGradient>
							<linearGradient id="paint2_linear_2421_900" x1="2.86521" y1="15.7524" x2="46.9493" y2="34.1113" gradientUnits="userSpaceOnUse">
							<stop stop-color="#FBA5E4"/>
							<stop offset="0.09" stop-color="#DC95DE"/>
							<stop offset="0.24" stop-color="#B280D6"/>
							<stop offset="0.37" stop-color="#9370D0"/>
							<stop offset="0.48" stop-color="#8067CD"/>
							<stop offset="0.56" stop-color="#7A64CC"/>
							<stop offset="0.64" stop-color="#6580AA"/>
							<stop offset="0.73" stop-color="#559690"/>
							<stop offset="0.81" stop-color="#4AA67D"/>
							<stop offset="0.9" stop-color="#43AF71"/>
							<stop offset="1" stop-color="#41B36E"/>
							</linearGradient>
							<linearGradient id="paint3_linear_2421_900" x1="0.0714264" y1="25" x2="50.0714" y2="25" gradientUnits="userSpaceOnUse">
							<stop stop-color="#6D5DCA"/>
							<stop offset="0.11" stop-color="#6D53B5"/>
							<stop offset="0.25" stop-color="#6E4AA3"/>
							<stop offset="0.4" stop-color="#6E4597"/>
							<stop offset="0.55" stop-color="#6F4494"/>
							<stop offset="1" stop-color="#ED693C"/>
							</linearGradient>
							<linearGradient id="paint4_linear_2421_900" x1="2.33115" y1="34.2851" x2="47.2946" y2="15.9262" gradientUnits="userSpaceOnUse">
							<stop stop-color="#D6D6D6"/>
							<stop offset="0.01" stop-color="#D1D5D6"/>
							<stop offset="0.13" stop-color="#9FCEDA"/>
							<stop offset="0.25" stop-color="#78C9DD"/>
							<stop offset="0.35" stop-color="#5CC5E0"/>
							<stop offset="0.44" stop-color="#4BC2E1"/>
							<stop offset="0.51" stop-color="#45C2E2"/>
							<stop offset="0.55" stop-color="#5BC1D4"/>
							<stop offset="0.62" stop-color="#84BFBB"/>
							<stop offset="0.69" stop-color="#A7BEA6"/>
							<stop offset="0.76" stop-color="#C1BD95"/>
							<stop offset="0.84" stop-color="#D4BC8A"/>
							<stop offset="0.92" stop-color="#E0BC83"/>
							<stop offset="1" stop-color="#E4BC81"/>
							</linearGradient>
							<linearGradient id="paint5_linear_2421_900" x1="8.24424" y1="42.268" x2="41.7746" y2="7.85826" gradientUnits="userSpaceOnUse">
							<stop stop-color="#E4BC81"/>
							<stop offset="0.02" stop-color="#DDB684"/>
							<stop offset="0.19" stop-color="#B2949F"/>
							<stop offset="0.33" stop-color="#927BB2"/>
							<stop offset="0.46" stop-color="#7F6CBE"/>
							<stop offset="0.54" stop-color="#7967C3"/>
							<stop offset="0.61" stop-color="#765DB5"/>
							<stop offset="0.73" stop-color="#724FA3"/>
							<stop offset="0.86" stop-color="#6F4697"/>
							<stop offset="1" stop-color="#6F4494"/>
							</linearGradient>
							<linearGradient id="paint6_linear_2421_900" x1="15.8755" y1="47.5415" x2="33.9048" y2="3.3476" gradientUnits="userSpaceOnUse">
							<stop stop-color="#ED693C"/>
							<stop offset="0.52" stop-color="#41B36E"/>
							<stop offset="0.56" stop-color="#5BB471"/>
							<stop offset="0.63" stop-color="#84B675"/>
							<stop offset="0.7" stop-color="#A7B879"/>
							<stop offset="0.77" stop-color="#C1BA7D"/>
							<stop offset="0.84" stop-color="#D4BB7F"/>
							<stop offset="0.92" stop-color="#E0BB80"/>
							<stop offset="1" stop-color="#E4BC81"/>
							</linearGradient>
						</defs>
					</svg>
				</div>
			</div>
			<?php 
			echo '<div class="footer-menu flex-row">';
				foreach($footer_links as $item):
					$link = $item['link']; 
					echo '<a href="'.$link['url'].'">'.$link['title'].'</a>';
				endforeach;
			echo '</div>';
			echo '<div class="footer-social flex-row">';
				foreach($social_links as $item):
					$link = $item['link'];
					
					echo '<a href="'.$link['url'].'" title="'.$link['title'].'"><span class="icon">'.the_image($item['icon']).'</span></a>';
				endforeach;
			echo '</div>';
			?>
		</div><!-- .site-info -->

		<div class="site-footer__row container has-gutter">
			<p class="footer-terms text-sm text-center wrap:narrow">
				<?php echo $footer_terms; ?>
			</p>
		</div>
		<a class="beech-attr" href="https://beech.agency/?utm_medium=referral&utm_source=alpha1m&utm_campaign=Alpha1m%20attr%20link">
			Site by <span class="sr-only">Beech Agency</span>
			<svg id="beechLogo" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 649.26 134.76">
				<defs>
					<style>
					.logo-letter {
						fill: currentColor;
						stroke-width: 0;
						color: currentColor;
					}

					svg#beechLogo path {
						--_duration: 650ms;
						--_factor: 5;
						--_index: 1;
						animation: none;
						color: currentColor;
						transition: color 150ms ease;
					}

					svg#beechLogo path.logo-letter:nth-child(3) { --_index: 2; }
					svg#beechLogo path.logo-letter:nth-child(4) { --_index: 3; }
					svg#beechLogo path.logo-letter:nth-child(5) { --_index: 4; }
					svg#beechLogo path.logo-letter:nth-child(6) { --_index: 5; }

					.beech-attr:hover svg#beechLogo path {
						animation: beechColorChange var(--_duration) linear 2;
						animation-delay: calc(var(--_duration) / var(--_factor) * var(--_index));
					}

					@keyframes beechColorChange {
						0%, 19.9%   { color: #FF5D36; }
						20%, 39.9%  { color: #FFD140; }
						40%, 59.9%  { color: #F295D2; }
						60%, 79.9%  { color: #00AF9B; }
						80%, 99.9%  { color: #0089D8; }
						100%        { color: currentColor; }
					}
					</style>
				</defs>
			<path class="logo-letter" d="m84.22,131.55H0V3.21h84.22c25.51,0,42.03,14.28,42.03,30.64s-9.46,28.72-23.74,32.41c9.31,2.41,23.74,10.43,23.74,31.44,0,17.49-17.81,33.85-42.03,33.85Zm-12.35-89.52h-21.18v13.15h21.18c3.53,0,6.58-2.89,6.58-6.58s-3.05-6.58-6.58-6.58Zm0,37.7h-21.18v13.15h21.18c3.53,0,6.58-3.05,6.58-6.58s-3.05-6.58-6.58-6.58Z"></path>
			<path class="logo-letter" d="m134.6,131.55V3.21h115.99v38.82h-65.29v13.15h31.76v24.55h-31.76v13.15h65.29v38.66h-115.99Z"></path>
			<path class="logo-letter" d="m260.7,131.55V3.21h115.99v38.82h-65.29v13.15h31.76v24.55h-31.76v13.15h65.29v38.66h-115.99Z"></path>
			<path class="logo-letter" d="m448.88,134.76c-36.74,0-66.42-30.16-66.42-67.38S412.14,0,448.88,0c33.21,0,56.79,24.55,60.32,56.79h-41.71c-1.6-12.35-9.95-17.33-16.68-17.33-8.66,0-17.33,8.02-17.33,27.91s8.66,27.91,17.33,27.91c6.58,0,14.92-4.97,16.52-16.84h41.87c-3.53,32.09-27.27,56.31-60.32,56.31Z"></path>
			<path class="logo-letter" d="m598.56,131.55v-45.08h-30v45.08h-50.69V3.21h50.69v46.36h30V3.21h50.7v128.34h-50.7Z"></path>
  </svg>
		</a>
	</footer><!-- #colophon -->
</div><!-- #page -->

<div id="cursor-follower" class="cursor-follower">
	<span class="watch">WATCH</span>
	<span class="read">READ</span>
</div>

	<?php wp_footer(); ?>

	<script src="https://cdn.jsdelivr.net/npm/locomotive-scroll@beta/bundled/locomotive-scroll.min.js"></script>
	<script>
		const locomotiveScroll = new LocomotiveScroll();
		window.locomotiveScroll = locomotiveScroll;
	</script>

	<!--  THANKS MIKE! https://codepen.io/Mikhail-Bespalov/pen/MYwrMNy?editors=1100 -->
	<svg style="position:absolute;width:0;height:0">
		<filter id="frosted" primitiveUnits="objectBoundingBox">
			<feImage href="data:image/png;base64," x="0" y="0" width="1" height="1" result="map"/>
			<feGaussianBlur in="SourceGraphic" stdDeviation="0.02" result="blur"/>
			<feDisplacementMap id="disp" in="blur" in2="map" scale="1" xChannelSelector="R" yChannelSelector="G">
			<animate attributeName="scale" to="1.4" dur="0.3s" begin="btn.mouseover"  fill="freeze"/>
			<animate attributeName="scale" to="1" dur="0.3s" begin="btn.mouseout"   fill="freeze"/>
			</feDisplacementMap>
		</filter>
	</svg>
</body>
</html>
